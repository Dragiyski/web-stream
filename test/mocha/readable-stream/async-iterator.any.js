...
import { ReadableStream, CountQueuingStrategy } from '../../../index.js';
import { recordingReadableStream } from '../../resources/recording-streams.js';
import { flushAsyncEvents } from '../../resources/test-utils.js';
import { assert_unreached, promise_rejects_exactly, assert_throws_js, promise_test } from '../../assert.js';
...
function assert_iter_result(iterResult, value, done, message) {
    ...
    assert.strictEqual(Object.getPrototypeOf(iterResult), Object.prototype, `${prefix}[[Prototype]]`);
    assert.sameOrderedMembers(Object.getOwnPropertyNames(iterResult).sort(), ['done', 'value'], `${prefix}property names`);
    assert.strictEqual(iterResult.value, value, `${prefix}value`);
    assert.strictEqual(iterResult.done, done, `${prefix}done`);
}

suite('ReadableStream', () => {
    suite('[AsyncIterator]', () => {
        test('Async iterator instances should have the correct list of properties', () => {
            ...
            assert.strictEqual(Object.getPrototypeOf(Object.getPrototypeOf(it)), AsyncIteratorPrototype, 'prototype should extend AsyncIteratorPrototype');

            const methods = ['next', 'return'];
            for (const method of methods) {
                assert(method in it, 'should have all the correct methods');
            }

            for (const m of methods) {
                const propDesc = Object.getOwnPropertyDescriptor(it, m);
        ...
        promise_test('Async-iterating a push source', async () => {
            const s = new ReadableStream({
                start(c) {
                    c.enqueue(1);
                    c.enqueue(2);
            ...
            assert.sameOrderedMembers(chunks, [1, 2, 3]);
        });

        promise_test('Async-iterating a pull source', async () => {
            let i = 1;
            const s = new ReadableStream({
                pull(c) {
                    c.enqueue(i);
        ...
        promise_test('Async-iterating a push source with undefined values', async () => {
            const s = new ReadableStream({
                start(c) {
                    c.enqueue(undefined);
                    c.enqueue(undefined);
            ...
            assert.sameOrderedMembers(chunks, [undefined, undefined, undefined]);
        });

        promise_test('Async-iterating a pull source with undefined values', async () => {
            let i = 1;
            const s = new ReadableStream({
                pull(c) {
                    c.enqueue(undefined);
        ...
        promise_test('Async-iterating a pull source manually', async () => {
            let i = 1;
        ...
        promise_test('Async-iterating an errored stream throws', async () => {
            const s = new ReadableStream({
                start(c) {
                    c.error('e');
                }
        ...
        promise_test('Async-iterating a closed stream never executes the loop body, but works fine', async () => {
            const s = new ReadableStream({
                start(c) {
                    c.close();
                }
        ...
        promise_test('Async-iterating an empty but not closed/errored stream never executes the loop body and stalls the async function', async () => {
            const s = new ReadableStream();
        ...
        promise_test('Async-iterating a partially consumed stream', async () => {
            const s = new ReadableStream({
                start(c) {
                    c.enqueue(1);
                    c.enqueue(2);
            ...
            assert.sameOrderedMembers(chunks, [2, 3]);
        });
        ...
        for (const type of ['throw', 'break', 'return']) {
            for (const preventCancel of [false, true]) {
                test(`Cancellation behavior when ${type}ing inside loop body; preventCancel = ${preventCancel}`, async () => {
                    ...
                    if (preventCancel) {
                        assert.sameOrderedMembers(s.events, ['pull'], `cancel() should not be called`);
                    } else {
                        assert.sameOrderedMembers(s.events, ['pull', 'cancel', undefined], `cancel() should be called`);
                    }
                });
            }
        }
        ...
        for (const preventCancel of [false, true]) {
            promise_test(`Cancellation behavior when manually calling return(); preventCancel = ${preventCancel}`, async () => {
                const s = recordingReadableStream({
                    start(c) {
                        c.enqueue(0);
                    }
                ...
                if (preventCancel) {
                    assert.sameOrderedMembers(s.events, [], `cancel() should not be called`);
                } else {
                    assert.sameOrderedMembers(s.events, ['cancel', undefined], `cancel() should be called`);
                }
            });
        }

        promise_test(`next() rejects if the stream errors`, async t => {
            let timesPulled = 0;
            const s = new ReadableStream({
                pull(c) {
                    if (timesPulled === 0) {
        ...
        promise_test(`return() does not rejects if the stream has not errored yet`, async () => {
            let timesPulled = 0;
            const s = new ReadableStream({
                pull(c) {
                    if (timesPulled === 0) {
        ...
        promise_test(`return() rejects if the stream has errored`, async t => {
            const timesPulled = 0;
            const s = new ReadableStream({
                pull(c) {
        ...
        promise_test(`next() that succeeds; next() that reports an error; next()`, async t => {
            let timesPulled = 0;
            const s = new ReadableStream({
                pull(c) {
                    if (timesPulled === 0) {
        ...
        promise_test(`next() that succeeds; next() that reports an error(); next() [no awaiting]`, async () => {
            let timesPulled = 0;
            const s = new ReadableStream({
                pull(c) {
                    if (timesPulled === 0) {
            ...
            assert.strictEqual(iterResults[0].status, 'fulfilled', '1st next() promise status');
            assert_iter_result(iterResults[0].value, 0, false, '1st next()');

            assert.strictEqual(iterResults[1].status, 'rejected', '2nd next() promise status');
            assert.strictEqual(iterResults[1].reason, error1, '2nd next() rejection reason');

            assert.strictEqual(iterResults[2].status, 'fulfilled', '3rd next() promise status');
            assert_iter_result(iterResults[2].value, undefined, true, '3rd next()');
        });

        promise_test(`next() that succeeds; next() that reports an error(); return()`, async t => {
            let timesPulled = 0;
            const s = new ReadableStream({
                pull(c) {
                    if (timesPulled === 0) {
        ...
        promise_test(`next() that succeeds; next() that reports an error(); return() [no awaiting]`, async () => {
            let timesPulled = 0;
            const s = new ReadableStream({
                pull(c) {
                    if (timesPulled === 0) {
            ...
            assert.strictEqual(iterResults[2].status, 'fulfilled', 'return() promise status');
            assert_iter_result(iterResults[2].value, 'return value', true, 'return()');
        });

        promise_test(`next() that succeeds; return()`, async () => {
            let timesPulled = 0;
            const s = new ReadableStream({
                pull(c) {
        ...
        promise_test(`next() that succeeds; return() [no awaiting]`, async () => {
            let timesPulled = 0;
            const s = new ReadableStream({
                pull(c) {
                    c.enqueue(timesPulled);
            ...
            assert.strictEqual(iterResults[0].status, 'fulfilled', 'next() promise status');
            assert_iter_result(iterResults[0].value, 0, false, 'next()');

            assert.strictEqual(iterResults[1].status, 'fulfilled', 'return() promise status');
            assert_iter_result(iterResults[1].value, 'return value', true, 'return()');

            assert.strictEqual(timesPulled, 2);
        });

        promise_test(`return(); next()`, async () => {
        ...
        promise_test(`return(); next() [no awaiting]`, async () => {
            const rs = new ReadableStream();
            const it = rs.values();

            const iterResults = await Promise.allSettled([it.return('return value'), it.next()]);

            assert.strictEqual(iterResults[0].status, 'fulfilled', 'return() promise status');
            assert_iter_result(iterResults[0].value, 'return value', true, 'return()');

            assert.strictEqual(iterResults[1].status, 'fulfilled', 'next() promise status');
            assert_iter_result(iterResults[1].value, undefined, true, 'next()');
        });

        promise_test(`return(); return()`, async () => {
            const rs = new ReadableStream();
            const it = rs.values();
        ...
        promise_test(`return(); return() [no awaiting]`, async () => {
            const rs = new ReadableStream();
            const it = rs.values();

            const iterResults = await Promise.allSettled([it.return('return value 1'), it.return('return value 2')]);

            assert.strictEqual(iterResults[0].status, 'fulfilled', '1st return() promise status');
            assert_iter_result(iterResults[0].value, 'return value 1', true, '1st return()');

            assert.strictEqual(iterResults[1].status, 'fulfilled', '2nd return() promise status');
            assert_iter_result(iterResults[1].value, 'return value 2', true, '1st return()');
        });

        promise_test('values() throws if there\'s already a lock', () => {
            const s = new ReadableStream({
                start(c) {
                    c.enqueue(0);
                    c.close();
        ...
        promise_test(`Acquiring a reader after exhaustively async-iterating a stream`, async () => {
            const s = new ReadableStream({
                start(c) {
                    c.enqueue(1);
                    c.enqueue(2);
        ...
        promise_test(`Acquiring a reader after return()ing from a stream that errors`, async t => {
            let timesPulled = 0;
            const s = new ReadableStream({
                pull(c) {
                    if (timesPulled === 0) {
        ...
        promise_test(`Acquiring a reader after partially async-iterating a stream`, async () => {
            const s = new ReadableStream({
                start(c) {
                    c.enqueue(1);
                    c.enqueue(2);
            ...
            // read the first two assert_throws_jschunks, then cancel
            const chunks = [];
            for await (const chunk of s) {
                chunks.push(chunk);
                if (chunk >= 2) {
            ...
            assert.sameOrderedMembers(chunks, [1, 2]);

            const reader = s.getReader();
            await reader.closed;
        });

        promise_test(`Acquiring a reader and reading the remaining chunks after partially async-iterating a stream with preventCancel = true`, async () => {
            const s = new ReadableStream({
                start(c) {
                    c.enqueue(1);
                    c.enqueue(2);
        ...
        promise_test(`close() while next() is pending`, async () => {
            const rs = new ReadableStream({
                async start(c) {
                    c.enqueue('a');
                    c.enqueue('b');
            ...
            assert.sameOrderedMembers(chunks, ['a', 'b', 'c']);
        });
    });
});
